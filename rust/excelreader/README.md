...
|---|---:|---:|
| `open` | 80.7 µs | 95.4 µs |
| `parse_sheet` (2 or 6 bound columns) | 62.5 µs | 30.8 ms |
| typed chunks of 1,000 / 10,000 rows | - | 29.3 ms / 29.2 ms |
| `infer_schema` (sample 100 / 1,000 rows) | 98.7 µs | 674.2 µs |

`open` stays nearly flat across the 655x row-count jump (+18%) - XLSB's header carries its
...
|---|---:|---:|
| XLSX | 104.9 ms | 279.2 ms |
| XLSB | 60.7 ms | 85.5 ms |

ExcelReader is ~2.7x faster than calamine for XLSX and ~1.4x faster for XLSB on this workload -
...
| Benchmark | Median |
|---|---:|
| `columns` (`write_columns`, pre-transposed) | 46.7 ms |
| `sheet` (`write_sheet`, from `Vec<Row>`) | 52.0 ms |
...
about 11%: not free, but a minority of the cost of producing the file.
