...
namespace ExcelReader.Cli
{
    ...
    // is a one-line-or-two adapter: the real work lives in CliCommands, which takes its
    ...
    // for sheets/schema's result, stderr for convert's progress) is redirected -
    ...
    internal sealed class Commands
    {
        ...
        // path: Path to the workbook (.xlsx, .xlsb, .xls or .csv).
        // password: Password for an encrypted .xlsx/.xlsb workbook.
        [Command("sheets")]
        ...
        // Writes a sheet out to another spreadsheet format, to a file or to standard output.
        // path: Path to the workbook (.xlsx, .xlsb, .xls or .csv).
        // sheet: -s, Sheet to convert, by name or zero-based index. Defaults to the first.
        // output: -o, File to write to. Writes to standard output when omitted.
        // format: -f, Output format: xlsx, xlsb, xls or csv. Defaults to --output's extension, or csv when writing to standard output.
        // delimiter: -d, CSV field separator. Ignored for every other format. Defaults to a comma.
        // password: Password for an encrypted .xlsx/.xlsb source workbook.
        [Command("convert")]
        ...
        // Prints the inferred column schema, as "index[TAB]name[TAB]type", with a trailing ? for nullable columns.
        // path: Path to the workbook (.xlsx, .xlsb, .xls or .csv).
        // sheet: -s, Sheet to inspect, by name or zero-based index. Defaults to the first.
        // headerRow: 1-based row to take column names from; 0 means the sheet has no header.
        // sampleSize: How many rows after the header to sample.
        // password: Password for an encrypted .xlsx/.xlsb workbook.
        [Command("schema")]
