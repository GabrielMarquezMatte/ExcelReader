...
{
    // Status codes returned by every exported C function. Mirrors include/excelreader.h.
    internal static class NativeStatus
    {
        ...
        // ABI revision returned by xl_abi_version. Mirrors XL_ABI_VERSION in include/excelreader.h.
        // Bump on any change to a struct layout, a status code, or the meaning of an existing function; adding a
        // new function does not bump it.
    ...
    // ExcelReader.Core.Reader.ExcelLimits holds for the managed side: a request naming more
    ...
    {
        // A..XFD — no sheet has more columns, so no spec list needs more entries.
        internal const int MaxColumnSpecs = 16_384;

        // Excel's 32,767-character cell limit at UTF-8's 4-byte worst case.
        internal const int MaxColumnNameBytes = 131_068;

        // Bounds how many candidate names a single xl_column_spec may carry.
        internal const int MaxNamesPerSpec = 32;
    }

    // Format selectors accepted by the open functions. Values 1-3 deliberately match
    // Core.Enums.ExcelFileFormat; CSV has no signature to sniff, so it has no
    // counterpart there and must always be requested explicitly. Note the enums are NOT
    // interchangeable beyond 3: managed 4 is Core.Enums.ExcelFileFormat.EncryptedOoxml
    // (a detection result) while native 4 is Csv (a request). Encryption is requested
