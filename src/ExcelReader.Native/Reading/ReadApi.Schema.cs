...
namespace ExcelReader.Native.Reading
{
    internal static unsafe partial class ReadApi
    {
        internal static int InferSchema(NativeHandle? handle, int headerRow, int sampleSize, out NativeInferredSchema schema)
        {
            ...
            if (headerRow < 0)
            {
                NativeApi.SetLastError($"header_row must be 0 (no header) or a positive row number; got {headerRow}.");
                return NativeStatus.InvalidArgument;
            }
            if (sampleSize <= 0)
            {
                NativeApi.SetLastError($"sample_size must be positive; got {sampleSize}.");
                return NativeStatus.InvalidArgument;
            }
