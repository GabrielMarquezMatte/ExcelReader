...
NativeAOT has no tiered JIT, so it never gets the dynamic PGO a JIT-hosted process does. That cost
...
`xl_parse_typed`, `xl_parse_typed_ex`, `xl_free_table`, `xl_infer_schema`, `xl_free_schema`,
