...
| `encrypted-ministream-sector-overrun.bin` | `encrypted` | `ArgumentOutOfRangeException` out of `CfbContainer.ReadMiniStream`. A mutated mini-FAT chain entry pointed at a sector whose *start* offset was validated as in-bounds, but whose full sector-sized slice ran past a truncated mini stream — the missing check let `Span.Slice` throw instead of the intended `InvalidDataException`. Now checked as `offset + take > miniStream.Length`; regression test in `ReaderLimitTests`. |
