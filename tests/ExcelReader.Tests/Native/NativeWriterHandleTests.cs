...
{
    /// <summary>
    /// Drives <see cref="WriteApi.OpenWriteHandle"/>/<see cref="WriteApi.CloseWriteHandle"/> and
    ...
    public sealed class NativeWriterHandleTests
    {
        ...
        {
            return WriteApi.OpenWriteHandle(Encoding.UTF8.GetBytes(path), format, options, out handle);
        }

        private static int OpenWriteHandleToMemory(int format, out NativeWriterHandle? handle, NativeWriteOptions options = default)
        {
            return WriteApi.OpenWriteHandleToMemory(format, options, out handle);
        }
        ...
        public void OpenWriteHandle_Should_Round_Trip_A_Streamed_Row_Through_OpenFile(int format, string extension)
        {
            ...
            try
            {
                ...
                Assert.Equal(NativeStatus.Ok, WriteApi.CloseWriteHandle(handle));

                Assert.Equal(NativeStatus.Ok, ReadApi.OpenFile(Encoding.UTF8.GetBytes(path), format, out NativeHandle? reader));
        ...
        public void OpenWriteHandle_Should_Reject_An_Empty_Path()
        {
            Assert.Equal(NativeStatus.InvalidArgument,
                WriteApi.OpenWriteHandle(ReadOnlySpan<byte>.Empty, NativeFormat.Xlsx, default, out NativeWriterHandle? handle));
            Assert.Null(handle);
        }
        ...
        public void CloseWriteHandle_Should_Produce_A_Valid_File_Even_When_EndSheet_Was_Never_Called()
        {
            ...
            try
            {
                ...
                Assert.Equal(NativeStatus.Ok, ReadApi.OpenFile(Encoding.UTF8.GetBytes(path), NativeFormat.Xlsx, out NativeHandle? reader));
                ReadApi.Close(reader);
            }
            finally
            {
        ...
        {
            Assert.Equal(NativeStatus.InvalidHandle, WriteApi.CloseWriteHandle(null));
        }
        ...
        public void CloseWriteHandle_Should_Not_Reuse_A_Handle_That_Was_Already_Closed()
        {
            ...
            try
            {
                ...
                int status = WriteApi.CloseWriteHandle(handle);
        ...
        public void NativeHandleTable_Should_Never_Resolve_A_Writer_Id_As_A_Reader_Or_Vice_Versa()
        {
            ...
            try
            {
                Assert.Equal(NativeStatus.Ok, ReadApi.OpenFile(Encoding.UTF8.GetBytes(xlsxFixture), NativeFormat.Auto, out NativeHandle? reader));
                ...
                ReadApi.Close(freedReader);
                writer!.StartSheet("S");
                Assert.Equal(NativeStatus.Ok, WriteApi.CloseWriteHandle(writer));
        ...
        public void GetWriteHandleBytes_Should_Round_Trip_A_Streamed_Row_Through_OpenMemory(int format)
        {
            ...
            Assert.Equal(NativeStatus.Ok, WriteApi.GetWriteHandleBytes(handle, out byte[]? bytes));
            Assert.NotNull(bytes);
            Assert.NotEmpty(bytes);

            Assert.Equal(NativeStatus.Ok, ReadApi.OpenMemory(bytes, format, out NativeHandle? reader));
            Assert.NotNull(reader);
            try
            {
                Span<byte> buffer = stackalloc byte[256];
        ...
        public void GetWriteHandleBytes_Should_Not_Require_EndSheet_First()
        {
            ...
            Assert.Equal(NativeStatus.Ok, ReadApi.OpenMemory(bytes, NativeFormat.Xlsx, out NativeHandle? reader));
            ReadApi.Close(reader);
        ...
        public void GetWriteHandleBytes_Called_Twice_Should_Return_The_Same_Content()
        {
            ...
            Assert.Equal(NativeStatus.Ok, WriteApi.GetWriteHandleBytes(handle, out byte[]? first));
            Assert.Equal(NativeStatus.Ok, WriteApi.GetWriteHandleBytes(handle, out byte[]? second));
        ...
        public void GetWriteHandleBytes_Should_Reject_A_File_Backed_Handle()
        {
            ...
            try
            {
                ...
                Assert.Equal(NativeStatus.InvalidArgument, WriteApi.GetWriteHandleBytes(handle, out byte[]? bytes));
                Assert.Null(bytes);
        ...
        {
            Assert.Equal(NativeStatus.InvalidHandle, WriteApi.GetWriteHandleBytes(null, out byte[]? bytes));
            Assert.Null(bytes);
        }
        ...
        public void CloseWriteHandle_After_GetWriteHandleBytes_Should_Not_Reopen_The_Workbook()
        {
            Assert.Equal(NativeStatus.Ok, OpenWriteHandleToMemory(NativeFormat.Xlsx, out NativeWriterHandle? handle));
            handle!.StartSheet("S");
            Assert.Equal(NativeStatus.Ok, WriteApi.GetWriteHandleBytes(handle, out _));
            Assert.Equal(NativeStatus.Ok, WriteApi.CloseWriteHandle(handle));
        }
    }
}
